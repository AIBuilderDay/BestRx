      ...
      <div className="w-full max-w-84 text-center">
        ...
        <form onSubmit={submit} noValidate className="text-left">
          <div className="mb-3.5">
            <label
              htmlFor="email"
              className="mb-1.25 block text-[11px] font-medium uppercase tracking-[0.07em] text-ink-3"
            >
              Email
            </label>
            <input
              ...
              className="w-full rounded-control border border-line-strong bg-bg px-2.75 py-2.25 text-ink placeholder:text-ink-3"
            />
          </div>
          <div className="mb-3.5">
            <label
          ...
          <button
            type="submit"
            className="w-full rounded-control border border-solid-bg bg-solid-bg px-3.5 py-2.25 text-[13px] font-medium text-solid-ink transition-opacity hover:opacity-85"
          >
            Sign in
          </button>
          {error ? (
        ...
        <div role="list" className="grid gap-2 text-left">
          {[...demoAccounts, ...demoFamily].map((user) => (
            <button
              ...
              className={`flex w-full items-center gap-2.75 rounded-card border px-3 py-2.25 transition-colors hover:bg-hover ${
                email === user.email ? 'border-ink' : 'border-line'
              }`}
            >
              <span
                aria-hidden="true"
                className="flex h-8 w-8 flex-none items-center justify-center rounded-full border border-line bg-bg-subtle text-[11px] font-semibold text-ink-2"
              >
                {initials(user.name)}
              </span>
