...
| `agent-cart-options.html` | AI agent → cart hand-off | Three animation options for the moment the AI agent places an item in the cart (wave hand-off ⭐, ocean flood, comet), using the decided ocean-sweep blues. Drawer auto-opens, agent-added line gets a "✦ Added by AI" chip. Pairs with `enhanced-search.html` |
